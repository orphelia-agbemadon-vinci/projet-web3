...
router.get('/history', (req, res) => {
    res.render('tasks/task_list', { tasks: tasksCompleted, lists, isHistory: true, isImportant: false, title: 'Historique' });
});

// Affiche la page avec les tâches marquées comme importantes
router.get('/important', (req, res) => {
    res.render('tasks/task_list', { tasks: tasksImportant, lists, isHistory: false, isImportant: true, title: 'Important' });
});
...
router.post('/history/add', (req, res) => {
    ...
    let newTask;
    if (taskDescription) {
        newTask = createTask(taskDescription);
    }
